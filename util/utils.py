...
import torch
from phase_correlation.loss import dice_loss
...
"""This module contains simple tool functions """
...
def imshow(tensor, title=None):
    ...
    # plt.imshow(image, cmap="gray").axes.get_xaxis().set_visible(False)
    plt.imshow(image, cmap="gray").axes.get_yaxis().set_visible(False)
...
def heatmap_imshow(tensor, name, title=None):
    image = tensor.cpu().detach().numpy(
    )  # we clone the tensor to not do changes on it
    ...
    fig = plt.gcf()
    plt.margins(0, 0)
    fig.savefig(name, dpi=500, bbox_inches='tight')  # dpi越高越清晰
    plt.show()
    ...
    # plt.margins(0, 0)
    # fig.savefig(name, dpi=500, bbox_inches='tight')  # dpi越高越清晰
    # out = torch.from_numpy(fig2data(fig))
...
def softargmax(input, device, beta=100):
    *_, h, w = input.shape
    ...
    input = input * 6000
    result = torch.sum(
        torch.exp(input).to(device) /
        torch.sum(torch.exp(input).to(device)).to(device) *
        torch.arange(0, h * w).to(device)).to(device)
    col = result // h
    row = result % col
...
def softargmax2d(input, device, beta=10000):
    *_, h, w = input.shape
    ...
    indices_c, indices_r = np.meshgrid(np.linspace(0, 1, w),
                                       np.linspace(0, 1, h),
                                       indexing='xy')
...
def GT_angle_convert(this_gt, size):
    for batch_num in range(this_gt.shape[0]):
        ...
        this_gt[batch_num] = this_gt[batch_num].clone() // 1 + (
            this_gt[batch_num].clone() % 1 + 0.5) // 1
...
def GT_scale_convert(scale_gt, logbase, size):
    for batch_num in range(scale_gt.shape[0]):
        scale_gt[batch_num] = torch.log10(
            1 / scale_gt[batch_num].clone()) / torch.log10(logbase) + 128.
...
def calc_loss(pred, target, metrics, bce_weight=0.5):
    ...
    metrics[
        'dice'] = metrics['dice'] + dice.data.cpu().numpy() * target.size(0)
    metrics[
        'loss'] = metrics['loss'] + loss.data.cpu().numpy() * target.size(0)
